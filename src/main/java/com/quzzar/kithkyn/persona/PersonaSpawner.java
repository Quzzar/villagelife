...
public final class PersonaSpawner {
    ...
    public static CompletableFuture<SpawnAttempt> trySpawn(ServerLevel level, BlockPos pos,
        ...
        if (person == null) {
            Kithkyn.LOGGER.error("Could not create a person entity for a persona spawn attempt");
        ...
        PersonaService.generateFor(person).whenComplete((result, error) -> level.getServer().execute(() -> {
            ...
            if (error != null) {
                Kithkyn.LOGGER.error("Persona generation errored for {}", name, error);
            }
            if (persona.isPresent()) {
                ...
                person.discard();
                Kithkyn.LOGGER.info("Arrival skipped: persona generation failed for {}", name);
