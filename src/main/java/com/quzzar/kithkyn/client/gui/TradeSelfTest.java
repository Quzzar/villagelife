...
import com.quzzar.kithkyn.networking.SelectTradePacket;
 ...
 */
@EventBusSubscriber(modid = Kithkyn.MODID, value = Dist.CLIENT, bus = Bus.GAME)
public final class TradeSelfTest {

    private static final String MODE = normalise(System.getProperty("kithkyn.uitest"));
    ...
    private static void interactWithNearestMerchant(Minecraft client) {
        client.level.getEntities(client.player,
                ...
                e -> e instanceof com.quzzar.kithkyn.entities.RealPerson person
                        && person.getOccupation() == com.quzzar.kithkyn.village.Occupation.MERCHANT)
                .stream().findFirst()
    ...
    private static void check(String what, boolean ok) {
        ...
        Kithkyn.LOGGER.info("TRADE TEST {} : {}", ok ? "PASS" : "FAIL", what);
    }

    private static void log(String message) {
        Kithkyn.LOGGER.info("TRADE TEST .... : {}", message);
    }
    ...
    private static void finish() {
        ...
        if (checksRun == 0) {
            failed = true;
            Kithkyn.LOGGER.info("TRADE TEST FAIL : no checks ran at all");
        }
        Kithkyn.LOGGER.info("TRADE TEST DONE : {}",
