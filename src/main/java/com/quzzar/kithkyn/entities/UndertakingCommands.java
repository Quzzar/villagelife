...
import com.quzzar.kithkyn.entities.UndertakingData.State;
import com.quzzar.kithkyn.entities.UndertakingData.Undertaking;
import com.quzzar.kithkyn.entities.UndertakingData.Valence;
import com.quzzar.kithkyn.entities.UndertakingService.Op;
 ...
 *
 * <ul>
 *   <li>{@code /kkdev undertaking selftest} — the apply state machine, checked
 ...
 *   <li>{@code /kkdev undertaking audit} — the model, checked against scripted
...
public final class UndertakingCommands {
    ...
    /**
     * Builds the REAL live chat prompt ({@link com.quzzar.kithkyn.chat.PersonChatContext#assemble})
    ...
    private static int probe(CommandSourceStack source, net.minecraft.world.entity.Entity target, String message) {
        ...
        UUID tester = UUID.nameUUIDFromBytes("kithkyn-tester".getBytes());
        var chat = com.quzzar.kithkyn.chat.PersonChatContext.assemble(person, "Tester", tester,
                List.of(), message);
        ...
        LlmService.get().submitChat("undertaking probe on " + person.getFullName(), chat.system(), chat.user(),
                ...
                .thenAccept(raw -> {
                    ...
                    Kithkyn.LOGGER.info("[probe] offered={} emitted={} raw={}", offered, emitted, r);
     ...
     * write path can be eyeballed end to end after a {@code /kithkyn chat} turn:
    ...
    private static int show(CommandSourceStack source, net.minecraft.world.entity.Entity target) {
        ...
        List<Undertaking> all = person.getData(KithkynAttachments.UNDERTAKINGS.get()).undertakings();
        if (all.isEmpty()) {
        ...
        UUID tester = UUID.nameUUIDFromBytes("kithkyn-tester".getBytes());
        int standing = person.getData(KithkynAttachments.SOCIAL.get()).relationships().getOrDefault(tester, 0);
