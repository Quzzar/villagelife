...
/**
 * Loads aptitude profiles from {@code data/<namespace>/kithkyn/aptitude/*.json}
 * into {@link JobAptitudes}. Mirrors {@link com.quzzar.kithkyn.village.tiers.VillageTierLoader}.
...
public class JobAptitudeLoader extends SimpleJsonResourceReloadListener {
  ...
  public JobAptitudeLoader() {
    super(new Gson(), "kithkyn/aptitude");
  }
  ...
  protected void apply(Map<ResourceLocation, JsonElement> jsons, ResourceManager resourceManager,
    ...
    jsons.forEach((id, json) -> Raw.CODEC.parse(JsonOps.INSTANCE, json)
        .resultOrPartial(error -> Kithkyn.LOGGER.error("Invalid aptitude profile {}: {}", id, error))
        .ifPresent(raw -> {
          try {
            ...
            Kithkyn.LOGGER.error("Aptitude profile {} names an unknown occupation or stat: {}", id,
                e.getMessage());
          }
        }));
    JobAptitudes.reload(loaded);
    Kithkyn.LOGGER.info("Loaded {} job aptitude profiles", loaded.size());
  }
}
