...
public final class VillageNamer {
  ...
  public static void requestFoundingName(ServerLevel level, BlockPos center, Consumer<String> onName) {
    ...
    String user = foundingPrompt(biome);
    request(level, system, user, true, onName);
  }

  /** Builds location-only context so temporary founding structures cannot bias the name. */
  static String foundingPrompt(String biome) {
    return "A new settlement is being founded in the " + biome
        + " landscape. Give it a place-name inspired only by the natural surroundings and terrain.";
  }
