...
public class LlmEvents {
  ...
  /** Operator command: is the villager AI working? Lives on /kithkyn. */
  ...
  /** Operator command: retry after a failure. Lives on /kithkyn. */
  ...
  /** LLM test harness, mounted under /kkdev: superseded for players by talking to a villager. */
  ...
  private static int plan(CommandSourceStack source, net.minecraft.world.entity.Entity target) {
    if (!(target instanceof com.quzzar.kithkyn.entities.RealPerson person)) {
      source.sendFailure(Component.literal("Target is not a villager."));
      return 0;
    }
    com.quzzar.kithkyn.village.Village village = person.getVillage();
    if (village == null) {
      source.sendFailure(Component.literal("That villager belongs to no village."));
      return 0;
    }
    if (com.quzzar.kithkyn.village.Storehouse.chests(person).isEmpty()) {
    ...
    if (!llm.isReady()) {
      source.sendFailure(Component.literal(
          "LLM is not ready (status: " + llm.getStatus() + "). Try /kithkyn load."));
      return 0;
    }
    ...
    com.quzzar.kithkyn.village.QuartermasterPlanner.plan(person).thenAccept(outcome -> {
      ...
      server.execute(() -> {
        ...
        com.quzzar.kithkyn.village.QuartermasterPlanner.Outcome result = outcome.get();
        com.quzzar.kithkyn.village.ShelvingPlan.store(village, result.plan());
        com.quzzar.kithkyn.entities.ai.goals.work.ConsolidateStep.tidyStorehouse(person);
  ...
  private static void reportPlan(CommandSourceStack source,
      com.quzzar.kithkyn.entities.RealPerson person,
      com.quzzar.kithkyn.village.QuartermasterPlanner.Outcome result) {
    source.sendSuccess(() -> Component.literal(
        person.getFullName() + ": “" + result.note() + "”"), false);
    for (com.quzzar.kithkyn.village.ShelvingPlan.Category category : result.plan().categories()) {
  ...
  private static int say(CommandSourceStack source, net.minecraft.world.entity.Entity target, String message) {
    ...
    com.quzzar.kithkyn.chat.VillagerConversation.speakTest(person, message);
  ...
  private static int talk(CommandSourceStack source, net.minecraft.world.entity.Entity first,
    ...
    if (!(first instanceof com.quzzar.kithkyn.entities.RealPerson a)
        || !(second instanceof com.quzzar.kithkyn.entities.RealPerson b)) {
      source.sendFailure(Component.literal("Both targets must be villagers."));
      return 0;
    }
    if (com.quzzar.kithkyn.chat.VillagerConversation.tryStart(a, b, true)) {
  ...
  private static int chat(CommandSourceStack source, net.minecraft.world.entity.Entity target, String message) {
    ...
    LlmService llm = LlmService.get();
    if (!llm.isReady()) {
      source.sendFailure(Component.literal("LLM is not ready (status: " + llm.getStatus() + "). Try /kithkyn load."));
      return 0;
    }
    ...
    java.util.UUID speakerUUID = source.getEntity() != null
        ...
        : java.util.UUID.nameUUIDFromBytes("kithkyn-tester".getBytes());

    source.sendSuccess(() -> Component.literal("Thinking..."), false);
    long start = System.currentTimeMillis();
    MinecraftServer server = source.getServer();
    com.quzzar.kithkyn.chat.PersonChatDispatcher.converse(person, speakerName, speakerUUID, message)
  ...
  private static int ask(CommandSourceStack source, String query) {
    ...
    if (parts.size() < 3) {
      source.sendFailure(Component.literal("Format: /kkdev llm ask <situation> | <option 1> | <option 2> [| more options]"));
      return 0;
    }
    LlmService llm = LlmService.get();
    if (!llm.isReady()) {
    ...
    long start = System.currentTimeMillis();
    MinecraftServer server = source.getServer();
    llm.decide("/kkdev llm ask", situation, options).thenAccept(decision -> server.execute(() -> {
      long tookMs = System.currentTimeMillis() - start;
