...
public final class OpenAiCompatibleProvider implements LlmProvider {
   ...
   * @param sendSamplingControls   send temperature and frequency_penalty;
   *                               models with fixed sampling defaults reject
   *                               both fields even when their values look
   *                               otherwise valid
  ...
  public record Spec(String name, String baseUrl, String defaultModel,
      ...
      boolean sendSamplingControls, boolean keyless) {
  }
  ...
  public static final Spec OPENAI = new Spec("OpenAI", "https://api.openai.com", "gpt-5.6-luna",
      true, false, false, false);
  public static final Spec DEEPSEEK = new Spec("DeepSeek", "https://api.deepseek.com", "deepseek-chat",
  ...
  public void start() {
    if (!spec.keyless() && (apiKey.get() == null || apiKey.get().isBlank())) {
      statusDetail = "no API key configured for " + spec.name()
          + ". Paste your key into kithkyn-common.toml (treat that file like a password).";
  ...
  public CompletableFuture<Optional<String>> complete(CompletionRequest request) {
    return attempt(requestBody(request).toString(), 0);
  }

  JsonObject requestBody(CompletionRequest request) {
  ...
  private CompletableFuture<Optional<String>> attempt(String body, int tryNumber) {
    ...
    return http.sendAsync(httpRequest, HttpResponse.BodyHandlers.ofString())
        ...
        .exceptionallyCompose(t -> {
          Kithkyn.LOGGER.warn("{} request failed: {}", spec.name(), t.getMessage());
  ...
  private CompletableFuture<Optional<String>> handleResponse(HttpResponse<String> response, String body,
    ...
    if (code == 200) {
      try {
        ...
        if (text.isBlank() && "length".equals(finishReason)) {
          Kithkyn.LOGGER.warn("{}: reply truncated before any visible text (finish_reason=length)", spec.name());
          return CompletableFuture.completedFuture(Optional.empty());
        }
        return CompletableFuture.completedFuture(text.isBlank() ? Optional.empty() : Optional.of(text));
      } catch (Exception e) {
        Kithkyn.LOGGER.warn("{}: unparseable response body", spec.name(), e);
        return CompletableFuture.completedFuture(Optional.empty());
      }
    }
    ...
    if (code == 429 || code >= 500) {
      Kithkyn.LOGGER.warn("{}: HTTP {} ({}), retrying", spec.name(), code, errorCode);
      return retryOrEmpty(body, tryNumber);
    }
    Kithkyn.LOGGER.warn("{}: HTTP {} ({})", spec.name(), code, errorCode);
    return CompletableFuture.completedFuture(Optional.empty());
  }
