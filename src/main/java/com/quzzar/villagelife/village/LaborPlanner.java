...
public final class LaborPlanner {
  ...
  private static boolean isHungry(Village village) {
    ...
    return report != null
        && report.foodCount() < report.population() * VillagelifeConfig.AttractivenessFoodTargetPerCapita;
  }
  ...
  private static void apply(Village village, ServerLevel level, List<UUID> crewIds, String field,
    ...
    if (error != null) {
      Villagelife.LOGGER.error("[labor] '{}' could not decide who to put on the field", village.getName(), error);
      return;
    }
    ...
    if (decision.choiceIndex() < 0 || decision.choiceIndex() >= crewIds.size()) {
      ...
      Villagelife.LOGGER.info("[labor] '{}' leaves its crew as it is: {}", village.getName(), decision.reason());
      return;
    }
