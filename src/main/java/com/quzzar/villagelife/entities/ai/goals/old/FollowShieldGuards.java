...
public class FollowShieldGuards extends Goal {
    private static final TargetingConditions NEARBY_GUARDS = TargetingConditions.forNonCombat().range(8.0D)
            .ignoreLineOfSight();
    private final RealPerson taskOwner;
    private RealPerson guardtofollow;
    private double x;
    private double y;
    private double z;

    public FollowShieldGuards(RealPerson taskOwnerIn) {
        this.taskOwner = taskOwnerIn;
    }

    @Override
    public boolean canUse() {
        List<? extends RealPerson> list = this.taskOwner.level.getEntitiesOfClass(this.taskOwner.getClass(),
                this.taskOwner.getBoundingBox().inflate(8.0D, 8.0D, 8.0D));
        if (!list.isEmpty()) {
            for (RealPerson guard : list) {
                if (!guard.isInvisible() && guard.getOffhandItem().canPerformAction(net.minecraftforge.common.ToolActions.SHIELD_BLOCK) && guard.isBlocking()
                        && this.taskOwner.level
                                .getNearbyEntities(RealPerson.class, NEARBY_GUARDS.range(3.0D), guard,
