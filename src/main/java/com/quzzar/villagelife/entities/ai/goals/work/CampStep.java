...
public final class CampStep implements BlockWorkStep {
  ...
  public boolean act(RealPerson person, BlockPos target) {
    ...
    if (campfire == null) {
      ...
      Villagelife.LOGGER.debug("[road] '{}' found no lit fire at their camp {} ({})", person.getFullName(),
  ...
  private boolean pitch(RealPerson person, ServerLevel level) {
    ...
    if (spot == null) {
      Villagelife.LOGGER.debug("[road] '{}' wants to camp but finds no level ground at {}",
