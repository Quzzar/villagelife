...
public class RaiseShieldGoal extends Goal {
    ...
    public boolean canUse() {
        return !CrossbowItem.isCharged(guard.getMainHandItem()) && (guard.getOffhandItem().getItem().canPerformAction(guard.getOffhandItem(), net.neoforged.neoforge.common.ItemAbilities.SHIELD_BLOCK) && raiseShield() && guard.shieldCoolDown == 0
                && !guard.getOffhandItem().getItem().equals(BuiltInRegistries.ITEM.get(ResourceLocation.parse("bigbrain:buckler"))));
    }
    ...
    @Override
    public void start() {
        if (guard.getOffhandItem().getItem().canPerformAction(guard.getOffhandItem(), net.neoforged.neoforge.common.ItemAbilities.SHIELD_BLOCK))
