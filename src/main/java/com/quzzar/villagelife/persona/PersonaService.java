...
public final class PersonaService {
    ...
    public static CompletableFuture<Optional<PersonaData>> generateFor(RealPerson person) {
        ...
        return attempt(person, start).thenCompose(first -> {
            ...
            Villagelife.LOGGER.debug("Retrying persona generation for {}", person.getFullName());
    ...
    private static CompletableFuture<Attempt> attempt(RealPerson person, long start) {
        ...
        return LlmService.get().submitPersona(PersonaPrompts.SYSTEM, sheet,
                ...
                .thenApply(raw -> {
                    ...
                    if (parsed.isEmpty()) {
                        Villagelife.LOGGER.warn("Persona generation for {} produced unparseable output: {}",
