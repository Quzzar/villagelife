...
import com.quzzar.villagelife.relationships.BirthMultiplicity;
import com.quzzar.villagelife.relationships.ChildCreationService;
import com.quzzar.villagelife.relationships.FamilyPlanningService;
...
public final class FamilyCommands {
  ...
  private static int consider(CommandSourceStack source, Entity firstEntity, Entity secondEntity) {
    ...
    FamilyPlanningService.considerNow(village, level, first, second).thenAccept(decision -> {
      String result = decision.map(value -> value == com.quzzar.villagelife.relationships.FamilyPlanningDialogue.Decision.WANT_CHILD
  ...
  private static int birth(
    ...
    FamilyPlanningService.birthNow(level, first, second, multiplicity).whenComplete((attempts, error) -> {
      if (error != null) {
        Villagelife.LOGGER.error("Family development birth failed", error);
