 ...
 * a time toward a walkable slope, and every move is checked against the
 * height recorded here: no column ever stands more than the levelling budget
 * above or below where it started, and a column is never moved back across
 * its own starting height. Without this reference a long hillside would be
 * regraded one step at a time into a plateau, each move locally reasonable.
 *
...
public final class GradedColumnStore extends SavedData {
  ...
  private static final String DATA_NAME = Villagelife.MODID + "~graded_columns";
